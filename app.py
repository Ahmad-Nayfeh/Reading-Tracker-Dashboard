...
import auth_manager # Import the auth manager
from googleapiclient.discovery import build # Needed to build the Forms API service
...
def generate_date_options():
    """
    Generates a list of the last 7 days for the dropdown menu
    with proper Arabic day names.
    """
    # Set locale to Arabic for day names, with a fallback
    try:
    ...
    for i in range(7):
        ...
        english_day_name = current_date.strftime('%A')
        arabic_day_name = arabic_days.get(english_day_name, english_day_name)
        date_str = current_date.strftime("%Y-%m-%d")
        dates.append(f"{date_str} ({arabic_day_name})")
            
    return dates

# --- Page Configuration ---
st.set_page_config(
    page_title="لوحة تحكم تحدي القرّاء",
    page_icon="📚",
    layout="wide"
)
...
# If we are here, it means the user is successfully logged in.
...
# Step 2a: Create the Google Sheet
if not spreadsheet_url:
    ...
    if 'sheet_title' not in st.session_state:
        st.session_state.sheet_title = f"بيانات تحدي القراءة - {datetime.date.today().year}"
...
# Step 2b: Add Members and Create the Google Form
if not form_url:
                ...
                try:
                    ...
                    update_requests = {
                        "requests": [
                            ...
                            {"createItem": {"item": {"title": "تاريخ القراءة", "questionItem": {"question": {"required": True, "choiceQuestion": {"type": "DROP_DOWN", "options": [{"value": date} for date in date_options]}}}}, "location": {"index": 1}}},
                    ...
                    with st.spinner("تتم مزامنة الملف مع Google Drive..."):
                        time.sleep(7)
                    ...
                    if st.button("لقد قمت بربط النموذج، تابع إلى الخطوة الأخيرة!"):
                        with st.spinner("جاري تنظيف جدول البيانات..."):
                            try:
                                ...
                                st.warning(f"لم نتمكن من حذف الصفحة الفارغة تلقائياً: {e}.")
                        st.rerun()
                except Exception as e:
...
if not setup_complete:
    ...
    with st.form("new_challenge_form", clear_on_submit=True):
        ...
        st.number_input("سنة النشر", key="pub_year", value=datetime.date.today().year, step=1)
...
if page == "لوحة التحكم":
    ...
    if not challenge_options:
        st.info("لا توجد تحديات حالية. يمكنك إضافة تحدي جديد من صفحة 'الإضافات'.")
        st.stop()
    ...
    days_passed = (datetime.date.today() - start_date_obj).days + 1
    ...
    st.info("هذه هي الروابط الخاصة بمساحة العمل الحالية في حسابك على جوجل.")
    ...
    st.info("هنا يمكنك تعديل نظام حساب النقاط والخصومات الذي ينطبق على جميع التحديات.")
